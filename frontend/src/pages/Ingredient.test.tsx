...
test("that we are connected to category list", async () => {
    ...
    await waitFor(() => {
        ...
        expect(() => screen.getByTestId("button-field0")).not.toThrowError()
    })
    screen.getByTestId("button-field0").click()

    await waitFor(() => {
        expect(() => screen.getByTestId("dummy-category")).not.toThrowError()
        expect(screen.getByTestId("dummy-category").textContent).toEqual("Category")
    })
})
